...
# ── WARRIOR TRADING SCANNER CRITERIA (EXACT FROM YOUR IMAGES) ─────────────────
MIN_PRICE          = 3.00
MAX_PRICE          = 20.00
...
def get_snapshots(symbols: list[str]) -> dict:
    ...
    for i in range(0, len(symbols), BATCH_SIZE):
        ...
        try:
            resp = requests.get(url, headers=alpaca_headers(), params={"symbols": ",".join(batch)}, timeout=30)
            resp.raise_for_status()
...
def get_relative_volume(symbol: str, current_volume: int) -> float | None:
    """Exact 30-day average volume (Ross preferred)"""
    try:
        ...
        resp = requests.get(
            ...
            params={"timeframe": "1Day", "start": start.isoformat(), "end": end.isoformat(), "limit": 30, "feed": "iex"},
            timeout=15
        )
        resp.raise_for_status()
...
def classify_stock(symbol: str, snap: dict) -> dict | None:
    ...
    if percent_change < MIN_PERCENT_CHANGE: return None
    if volume < WATCH_MIN_VOLUME: return None
    if spread_pct is not None and spread_pct > MAX_SPREAD_PCT: return None

    rel_vol = get_relative_volume(symbol, volume)
    if rel_vol is None or rel_vol < MIN_REL_VOLUME: return None
    ...
    reason = f"chg:{percent_change:.1f}% | vol:{volume:,} | rvol:{rel_vol}x | spread:{spread_pct:.2f}%"
...
def run_scanner():
    ...
    print(f"[SCANNER] Found {len(candidates)} candidates (A_SETUP requires news + low float)")

    supabase.table("bot_watchlist").upsert(candidates, on_conflict="symbol").execute()
    print(f"[DB] Upserted {len(candidates)} rows with news headlines.")
